...
<#
    if(($currentprogramname -eq "PMACU") -and ($currentprogramnameyear -like "2017") -and ($oldprogramlookup -eq "BA")){
    ...
    }elseif (($currentprogramname -eq "PMAHA") -and ($currentprogramnameyear -like "2017") -and ($oldprogramlookup -eq "BA")) {
        Get-AdPrincipalGroupMembership -Identity $currentusername | Where-Object {($_.name -ne 'Domain Users')} | Remove-AdGroupMember -Members $currentusername -Confirm:$false
        Add-ADGroupMember -Identity "All Students" -Members $currentusername
        Add-ADGroupMember -Identity "#Students-MA-HA" -Members $currentusername
        Get-ADUser -Identity $currentusername | Move-ADObject -TargetPath "OU=20,OU=MAHist,OU=HIST,OU=Students,OU=Courtauld Users,DC=institute,DC=courtauld,DC=local"
    ...
    }elseif (($currentprogramname -eq "CGDHA") -and ($currentprogramnameyear -like "2017") -and ($oldprogramlookup -eq "BA")) {
        Get-AdPrincipalGroupMembership -Identity $currentusername | Where-Object {($_.name -ne 'Domain Users')} | Remove-AdGroupMember -Members $currentusername -Confirm:$false
        Add-ADGroupMember -Identity "All Students" -Members $currentusername
        Add-ADGroupMember -Identity "#Students-CGDIP-HA" -Members $currentusername -ErrorAction SilentlyContinue
        Get-ADUser -Identity $currentusername | Move-ADObject -TargetPath "OU=20,OU=CGDipHA,OU=HIST,OU=Students,OU=Courtauld Users,DC=institute,DC=courtauld,DC=local"
    ...
    }elseif (($currentprogramname -eq "PGDCP") -and ($currentprogramnameyear -like "2017") -and ($oldprogramlookup -eq "BA")) {
        Get-AdPrincipalGroupMembership -Identity $currentusername | Where-Object {($_.name -ne 'Domain Users')} | Remove-AdGroupMember -Members $currentusername -Confirm:$false
        Add-ADGroupMember -Identity "All Students" -Members $currentusername
        Add-ADGroupMember -Identity "#Students-PGDIP-CE" -Members $currentusername -ErrorAction SilentlyContinue
        Add-ADGroupMember -Identity "StuEasels" -Members $currentusername -ErrorAction SilentlyContinue
    ...
    }elseif (($currentprogramname -eq "PMABC") -and ($currentprogramnameyear -like "2017") -and ($oldprogramlookup -eq "BA")) {
        Get-AdPrincipalGroupMembership -Identity $currentusername | Where-Object {($_.name -ne 'Domain Users')} | Remove-AdGroupMember -Members $currentusername -Confirm:$false
        Add-ADGroupMember -Identity "#Students-MA-BU" -Members $currentusername
        Add-ADGroupMember -Identity "StuBUMA" -Members $currentusername
        Add-ADGroupMember -Identity "All Students" -Members $currentusername
    ...
    }elseif (($currentprogramname -eq "PMACN") -and ($currentprogramnameyear -like "2017") -and ($oldprogramlookup -eq "BA")) {
        Get-AdPrincipalGroupMembership -Identity $currentusername | Where-Object {($_.name -ne 'Domain Users')} | Remove-AdGroupMember -Members $currentusername -Confirm:$false
        Add-ADGroupMember -Identity "#Students-MA-WA" -Members $currentusername
        Add-ADGroupMember -Identity "StuWalls" -Members $currentusername
        Add-ADGroupMember -Identity "All Students" -Members $currentusername
...
foreach ($user in $fileimport){
    ...
    if(((get-Userlookup $user.c_idnumber) -eq 1)){
        ...
        if ((Useralreadyloogedlookup $user.surname) -eq 1){
            ...
            $userprocessafter = CurrentUserLookup $user.c_idnumber $user.programme $user.starting_year $userprocess.distinguishedname
    
            Outuserprocess $user.idnumber $user.c_idnumber $user.firstname $user.lastname $user.fullname $oulookup.distinguishedname $user.year $user.starting_year $Userattributeloader1.mail "Old Student" $userprocessafter  
        }else{
            ...
            New-ADUser -SamAccountName $user.c_idnumber -UserPrincipalName $useremailnameformat -Name $user.firstname -DisplayName $userfullnameformat -GivenName $userfullnameformat -Surname $user.lastname -Office "Student" -Description $user.idnumber -accountpassword (ConvertTo-SecureString "$password" -AsPlainText -force) -Enabled $True -PasswordNeverExpires $false -PassThru | Out-Null
...
if ($Global:Results.count -gt 0){
    ...
    Write-Host "Waiting to sync 1 Minutes..."
